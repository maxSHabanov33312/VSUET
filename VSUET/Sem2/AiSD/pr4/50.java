...
class Calculator {
    ...
    public void run() {
        ...
        while (true) {
            ...
            System.out.println("3. Выбрать конкретное вычисление из списка и вывести его");
            ...
            switch (choice) {
                ...
                case 3:
                    System.out.println("\nВыберите вычисление из списка:");
                    for (int i = 0; i < history.size(); i++) {
                        System.out.printf("%d. %s%n", i + 1, history.get(i));
                    }
                    int index = scanner.nextInt() - 1;
                    String expressionChosen = history.get(index);
                    double resultChosen = calculateExpression(expressionChosen);
                    System.out.printf("Результат выбранного вычисления (%s): %.2f%n", expressionChosen, resultChosen);
                    break;
